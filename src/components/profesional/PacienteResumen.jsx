import React from "react";

import { Card, Row, Col } from "react-bootstrap";

const PacienteResumen = ({ paciente }) => {
  if (!paciente) {
    return null;
  }

  const calcularEdad = () => {
    return paciente?.edad || "--";
  };

  return (
    <Card className="paciente-banner mb-4">
      <Card.Body>
        <div className="paciente-header">
          <div>
            <h3 className="mb-1">
              {paciente?.nombres} {paciente?.apellidos}
            </h3>

            <p className="text-muted mb-0">RUT: {paciente?.numeroDocumento}</p>
          </div>

          <div className="estado-paciente">Paciente activo</div>
        </div>

        <Row className="mt-4 g-3">
          <Col md={4}>
            <div className="paciente-info-card">
              <small className="text-muted">Edad</small>

              <h5>{paciente?.edad || calcularEdad()}</h5>
            </div>
          </Col>

          <Col md={4}>
            <div className="paciente-info-card">
              <small className="text-muted">Género</small>

              <h5>{paciente?.genero || "No registrado"}</h5>
            </div>
          </Col>

          <Col md={4}>
            <div className="paciente-info-card">
              <small className="text-muted">Peso actual</small>

              <h5>
                {paciente?.antropometria?.length > 0
                  ? `${paciente.antropometria[paciente.antropometria.length - 1].peso} kg`
                  : "No registrado"}
              </h5>
            </div>
          </Col>

          <Col md={12}>
            <div className="paciente-info-card">
              <small className="text-muted">Alergias</small>

              <h5>{paciente?.alergias || "Sin alergias"}</h5>
            </div>
          </Col>
        </Row>
      </Card.Body>
    </Card>
  );
};

export default PacienteResumen;
